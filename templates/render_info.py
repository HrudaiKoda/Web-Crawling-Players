...
def is_valid_string(attribute_value):
    if not isinstance(attribute_value, str):
        return True
    return not (attribute_value == None or pd.isnull(attribute_value) or str(attribute_value) == "" or str(
        attribute_value) == "nan")
...
def get_teams_string(teams_list, Nationality):
    ...
    capitals = {
        ...
        'Patriots 2': 'పేట్రియాట్స్ 2', 'UCCE': 'యూ.సీ.సీ.ఇ.', 'Middlesex 2nd': 'మిడిల్‌సెక్స్ 2న్ద్',
        'Under-23': 'అండర్-23',
        'D.A.V': 'డి.ఏ.వీ.', 'HDG': 'హెచ్.డి.జీ.', 'CC': 'సీ.సీ.', 'MAS': 'ఎం.ఏ.ఎస్.', 'ACB': 'ఏ.సీ.బీ.',
        'UAE': 'యూ.ఏ.ఇ.',
        'Under 19': 'అండర్ 19', '2': '2', 'MC': 'ఎం.సీ.', 'AJK': 'ఏ.జె.కె.', 'CI': 'సీ.ఐ.', 'TUKS': 'టి.యూ.కె.ఎస్.',
        'Pakhtunkhwa 2nd': 'పఖ్తున్ఖ్వా 2న్ద్', 'Sindh 2nd': 'సింధ్ 2న్ద్', 'DOHS': 'డి.ఓ.హెచ్.ఎస్.',
        'TUTI': 'టి.యూ.టి.ఐ.',
        'Gloucestershire 2nd': 'గ్లౌసెస్టర్షైర్ 2న్ద్', 'Hampshire 2nd': 'హాంప్‌షైర్ 2న్ద్', 'D': 'డి.',
        'Derbyshire 2nd': 'డెర్బీషైర్ 2న్ద్',
...
def get_awards(teams_list):
    ...
    actual_list = ast.literal_eval(teams_list)
    capitals = {
        'OBE': 'ఓ.బీ.ఇ.', 'july': "జూలై", 'Year': 'ఇయర్', 'year': 'ఇయర్', 'on': 'ఆన్', 'Dec': 'డిసెంబర్', 'the': 'ది',
        'The': "ది", 'MBE': 'ఎం.బీ.ఇ.', 'SASSSA': 'ఎస్.ఏ.ఎస్.ఎస్.ఎస్.ఏ.',
        ...
        'Under': 'అండర్', 'EL': 'ఇ.ఎల్.', 'BBC': 'బీ.బీ.సీ.', 'FR': '', 'GO': 'ఎఫ్.ఆర్.', 'MTN': 'ఎం.టి.ఎన్.',
        'PCA': 'పి.సీ.ఏ.',
        'S.F.': 'ఎస్.ఎఫ్.', 'PE': 'పి.ఇ.', 'SAARC': 'ఎస్.ఏ.ఏ.ఆర్.సీ.', 'GW': 'జీ.డబల్యూ.', 'OFS': 'ఓ.ఎఫ్.ఎస్.',
        'ODI': 'ఓ.డి.ఐ.',
        'Surrey': 'సర్రే', 'Under-17s': 'అండర్ -17స్', 'Derbyshire': 'డెర్బీషైర్', 'A': 'ఏ.', 'KS': 'కె.ఎస్.',
        'S': 'ఎస్.',
        'Lancashire': 'లాంక్షైర్', 'Leicestershire': 'లీసెస్టర్షైర్', 'NT': 'ఎన్.టి.', 'SRdeS': 'ఎస్.ఆర్.డి.ఇ.ఎస్.',
        'T20I': 'టి20ఐ', 'GRADE': 'జీ.ఆర్.ఏ.డి.ఇ.', 'NBC': 'ఎన్.బీ.సీ.', 'I': 'ఐ.', 'EP': 'ఇ.పి.', 'CC': 'సీ.సీ.',
        'DR': 'డి.ఆర్.', 'St': 'సెయింట్'
    }
    translated_output = ""
    twos = [ke for ke in capitals.keys() if len(ke.split(" ")) == 2]
    ones = [ke for ke in capitals.keys() if len(ke.split(" ")) == 1]
...
def getData(row, countries):
    ...
    bith_overview = row['Birth_Date'].values[0]
    if birth_date != 'nan':
...
def main():
    ...
    glob = {'get_profile_ref': get_profile_ref, 'get_source': get_source,
            'get_trophy_names_list': get_trophy_names_list, 'get_trophy_name': get_trophy_name, 'conv': conv,
            'get_teams_string': get_teams_string, 'get_nation': get_nation}
    ...
    with open('infoandoverview.txt', 'w', encoding='utf-8') as fobj:
        ...
        row = moviesDF.loc[moviesDF['Cricinfo_id'] == 54950]
